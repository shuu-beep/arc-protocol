...
The baseline happy path above is now runnable, together with the failure catalog below. `episode.py` generates each run as a signed ARC event log and folds the log back, with nothing about the transaction stored.
