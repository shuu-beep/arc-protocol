...
> **Status:** Runnable slices landed — `episode.py` runs the baseline happy path plus five failure runs: stale-offer, payment-failure, colluding-reputation-farming, fake-merchant, and compromised-consumer-agent (see §5.1). The remaining failure-run artifacts are still mock JSON, not yet executable.
>
...
This directory began as a tiny mock reference flow plus JSON artifacts; the baseline happy path and the stale-offer, payment-failure, colluding-reputation-farming, fake-merchant, and compromised-consumer-agent failure runs are now runnable as `episode.py` (§5.1). The remaining failure-run artifacts are still mock JSON.
