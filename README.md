...
interpretive fidelity, temporal fidelity, world fidelity, and presentation
fidelity), ARC leaves visible rather than hidden.
...
These are not only claims on paper. A dozen small executable probes test them — each a single-purpose, dependency-light slice, not an implementation. A representative few:
...
- [`examples/end-to-end-demo`](examples/end-to-end-demo/) — four parties (human, consumer agent, merchant agent, community) each sign their own events; the standing projection moves only when an `ADJUDICATE` is added, never by mutating stored state, and nothing in the log is written by hand. Its `agent_flow.py` then hands the consumer side to an **actual reasoning agent** — a real Claude model when configured (verified once end-to-end on `claude-opus-4-8`), a scripted policy otherwise — holding only its own key and acting only through canonical-event-emitting tools. It cannot mint the human's `AUTHORIZE`, and the fold's payment audit catches a re-aimed payee or an uncovered, forged-approval spend: the agent's *reasoning* never widens its *authority* (findings K/L against a real reasoner, not a hand-written flow).
