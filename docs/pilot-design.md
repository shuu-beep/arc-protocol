...
- **Demonstrated:** the fold this instrument relies on is exercised on synthetic records in [`examples/refusal-recording-demo`](../examples/refusal-recording-demo/) — the schema yields a falsification surface (candidates contradicted in the cells they claim, or named as a gap, never validated) before any real refusal is collected.
