...
- **A compromised signer.** §2 relocates trust from the runtime to the signer; it does not eliminate it. What bounds a signer that lies about its mandate check is open — the same residue as the approval-surface question it descends from. The embodiment fixture (§5) makes this concrete rather than smaller: process separation narrows the trusted base to the signer, so a signer compromise is the full stolen-key blast radius again — localized to one boundary, not removed.
...
A closing note on where the custody–authority–revocation axis stands. It now has five linked expressions, each holding the same decisions a different way: this document (the design decisions), an executable probe on real keys ([compromise_fixture.py](../examples/reference-client/compromise_fixture.py)), a visible projection (the reference client's compromise band), a static lock ([custody.ts](../examples/canon-ts/custody.ts)), and a runtime embodiment that splits the signer from the agent as separate processes ([embodiment_fixture.py](../examples/reference-client/embodiment_fixture.py)). None of the five closes the axis — each is a probe-backed or compiler-locked slice, not a resolution — and what §8 left open stays open: a compromised signer, threshold custody, enclave attestation, detection latency.
