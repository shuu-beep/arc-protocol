...
No new compromise machinery is introduced; the compromise model *is* the composition of revocation semantics and mandate scope. This is the one slice of custody that reduces to a probe — a replayed compromise whose damage provably equals the mandate's remaining scope — and it is marked here as future probe material rather than asserted as tested.
