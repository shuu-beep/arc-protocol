...
A closing note on where the custody–authority–revocation axis stands. It now has four linked expressions, each holding the same decisions a different way: this document (the design decisions), an executable probe on real keys ([compromise_fixture.py](../examples/reference-client/compromise_fixture.py)), a visible projection (the reference client's compromise band), and a static lock ([custody.ts](../examples/canon-ts/custody.ts)). None of the four closes the axis — each is a probe-backed or compiler-locked slice, not a resolution — and what §8 left open stays open: a compromised signer, threshold custody, enclave attestation, detection latency.
